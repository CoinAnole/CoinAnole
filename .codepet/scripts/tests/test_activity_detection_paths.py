...
class FakeRepo:
    def __init__(self, branches: list[SimpleNamespace], commits_by_branch: dict[str, list[SimpleNamespace]]) -> None:
        ...
        self.commit_calls: list[tuple[str, datetime, str]] = []

    def get_branches(self) -> list[SimpleNamespace]:
        return self._branches

    def get_commits(self, sha: str, since: datetime, author: str) -> list[SimpleNamespace]:
...
class FakeGithub:
    def __init__(self, token: str, repos: dict[str, object]) -> None:
        self.token = token
        self._repos = repos
